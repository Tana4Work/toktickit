...
import { Category, createTicket, fetchCategories, fetchRelatedSystems, RelatedSystem, TicketPriority, uploadAttachment } from "./api.js";
...
export default function CreateTicket() {
  ...
  async function submit(event: FormEvent) {
    ...
    if (attachmentError || files.length > 5) { if (!attachmentError) setAttachmentError("You can select at most five attachments."); return; }
    ...
    try {
      ...
      let failed = false;
      for (const file of files) { try { await uploadAttachment(ticket.id, currentRequester.id, file); } catch { failed = true; } }
      setCreatedTicket(ticket); setAttachmentWarning(failed ? "Ticket created, but one or more attachments could not be uploaded." : ""); setSubmitState("success");
  ...
  return <section className="content-card" aria-labelledby="create-ticket-heading">
    <div className="section-heading"><div><p className="eyebrow">NEW REQUEST</p><h2 id="create-ticket-heading">Create Ticket</h2><p className="text-secondary mb-0">Submit a support request for the selected requester.</p></div></div>
    <p className="context-line">Requester: <strong>{currentRequester.name}</strong></p>
    ...
    {attachmentWarning && <p className="text-warning" role="alert">{attachmentWarning}</p>}
    <form className="ticket-form" onSubmit={submit} noValidate>
      ...
      <div className="mb-3"><label className="form-label" htmlFor="ticket-attachments">Attachments</label><input id="ticket-attachments" className="form-control" type="file" multiple accept=".jpg,.jpeg,.png,.webp,.pdf" onChange={(e) => { const selected = Array.from(e.target.files ?? []); const invalid = selected.find((file) => file.size > 5 * 1024 * 1024 || !["image/jpeg", "image/png", "image/webp", "application/pdf"].includes(file.type)); setFiles(selected.slice(0, 5)); setAttachmentError(invalid ? "Only JPG, JPEG, PNG, WEBP, and PDF files up to 5 MB are allowed." : selected.length > 5 ? "You can select at most five attachments." : ""); }} />{attachmentError && <div className="text-danger" role="alert">{attachmentError}</div>}{files.length > 0 && <small className="text-secondary">{files.length} attachment(s) selected.</small>}</div>
