...
export default function TicketDetail({ ticketId, onBack }) {
    ...
    async function handleUpload() {
        if (!currentRequester || !selectedFile)
            return;
        setUploading(true);
        setActionError("");
        try {
            ...
            const input = document.getElementById("existing-ticket-attachment");
            if (input)
                input.value = "";
            setTicket(await fetchTicket(ticketId, currentRequester.id));
        }
        catch (error) {
            setActionError(error instanceof Error ? error.message : "Unable to upload attachment.");
        }
        finally {
            setUploading(false);
        }
