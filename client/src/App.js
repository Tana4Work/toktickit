...
export default function App() {
    ...
    const [activePage, setActivePage] = useState(stored?.role === "Administrator" ? "users" : stored?.role === "ITStaff" ? "queue" : "tickets");
    const [selectedTicketId, setSelectedTicketId] = useState(null);
    const { selectRequester, clearRequester } = useRequester();
    ...
    function handleAuthenticated(authUser) { setUser(authUser); selectRequester(authUser); setSelectedTicketId(null); setActivePage(authUser.role === "Administrator" ? "users" : authUser.role === "ITStaff" ? "queue" : "tickets"); }
    ...
    const staffMode = user.role === "ITStaff" || user.role === "Administrator";
    return _jsxs("div", { className: "app-shell", children: [_jsxs("header", { className: "topbar", children: [_jsxs("div", { className: "brand", children: [_jsx("span", { className: "brand-mark", "aria-hidden": "true" }), _jsx("span", { children: "TokTickIT" })] }), _jsx("div", { className: "topbar-nav", "aria-label": "Primary navigation", children: staffMode ? _jsxs(_Fragment, { children: [_jsxs("button", { "aria-label": "My Queue", className: activePage === "queue" ? "topbar-link active" : "topbar-link", onClick: () => { setActivePage("queue"); setSelectedTicketId(null); }, children: ["\u2630 ", _jsx("span", { "aria-hidden": "true", children: "My Queue" })] }), user.role === "Administrator" && _jsxs("button", { "aria-label": "User Management", className: activePage === "users" ? "topbar-link active" : "topbar-link", onClick: () => { setActivePage("users"); setSelectedTicketId(null); }, children: ["\u2699 ", _jsx("span", { "aria-hidden": "true", children: "Users" })] })] }) : _jsxs(_Fragment, { children: [_jsxs("button", { "aria-label": "My Tickets", className: activePage === "tickets" ? "topbar-link active" : "topbar-link", onClick: () => { setActivePage("tickets"); setSelectedTicketId(null); }, children: ["\u2630 ", _jsx("span", { "aria-hidden": "true", children: "My Tickets" })] }), _jsxs("button", { "aria-label": "Create Ticket", className: activePage === "create" ? "topbar-link active" : "topbar-link", onClick: () => { setActivePage("create"); setSelectedTicketId(null); }, children: ["+ ", _jsx("span", { "aria-hidden": "true", children: "Create Ticket" })] })] }) }), _jsxs("div", { className: "requester-chip", children: [_jsx("span", { className: "user-mark", "aria-hidden": "true" }), _jsxs("span", { children: [user.name, " (", user.role, ")"] }), _jsx("button", { className: "change-requester-button", onClick: () => void handleLogout(), children: "Logout" })] })] }), _jsxs("main", { className: "page-content", children: [_jsx("button", { className: "visually-hidden", onClick: () => void handleCheck(), disabled: state === "loading", children: "Check System" }), state === "loading" && _jsx("p", { className: "mt-4 text-secondary", children: "Checking API status..." }), state === "success" && _jsxs("div", { className: "mt-4", children: [_jsx("p", { className: "text-success fw-semibold", children: "System Status: Online" }), _jsx("ul", { "aria-label": "Categories", children: categories.map((category) => _jsx("li", { children: category.name }, category.id)) })] }), state === "error" && _jsx("p", { className: "mt-4 text-danger", role: "alert", children: "System Status: Offline. Please try again." }), user.role === "Administrator" && activePage === "users" ? _jsx(AdminUserManagement, {}) : staffMode ? (selectedTicketId ? _jsx(StaffTicketDetail, { ticketId: selectedTicketId, onBack: () => setSelectedTicketId(null) }) : _jsx(StaffTicketQueue, { onOpenTicket: (ticketId) => setSelectedTicketId(ticketId) })) : _jsxs(_Fragment, { children: [activePage === "create" && _jsx(CreateTicket, {}), activePage === "tickets" && !selectedTicketId && _jsx(MyTickets, { onOpenTicket: (ticketId) => setSelectedTicketId(ticketId) }), activePage === "tickets" && selectedTicketId && _jsx(TicketDetail, { ticketId: selectedTicketId, onBack: () => setSelectedTicketId(null) })] })] })] });
}
